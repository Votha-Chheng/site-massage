...
const HomeBandeauFirst = () => {
  ...
  const [bandeauWidth, setBandeauWidth] = useState(0)
  ...
  useEffect(() => {
    setBandeauWidth(window.innerHeight)
  }, [size])

  useEffect(() => {
    if((positionY >= 1 && positionY < 101) && (scrollDir === "down") ){
      window.scrollTo(0, bandeauWidth)
    }
  }, [size, bandeauWidth, positionY, scrollDir])
